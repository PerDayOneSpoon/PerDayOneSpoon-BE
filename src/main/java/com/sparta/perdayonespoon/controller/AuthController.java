...
public class AuthController {
    ...
    @ApiImplicitParam(name = "refreshtoken", required = true,  dataType = "string", paramType = "header", value = "refreshtoken header")
    @ApiResponses({
            @ApiResponse(code = 200, message = "API 정상 작동",responseHeaders = @ResponseHeader(name = "refreshtoken", description = "refreshtoken이 담기는 헤더의 이름", response = TokenDto.class)),
            @ApiResponse(code = 400, message = "Request타입 에러, 토큰이 없을때 에러"),
            @ApiResponse(code = 401, message = "변조된 토큰 에러"),
            @ApiResponse(code = 408, message = "만료된 토큰 에러"),
            @ApiResponse(code = 500, message = "서버 에러")
    ...
    public ResponseEntity reissue( @RequestHeader(value = "refreshtoken", required = false) String refreshtoken){
        return googleService.regenerateToken(refreshtoken);
    }
}
